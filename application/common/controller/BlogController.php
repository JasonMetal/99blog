...
class BlogController extends Controller {
    ...
    public function __construct() {
        ...
        if(empty($this->keyword)){
            Db::table('keyword')->cache('keyword',6000)->select();
            $this->keyword = Cache::get('keyword');
        ...
        //     Db::table('blog_website_link')->cache('website_link',6000)->select();
        //     $this->website_link = Cache::get('website_link');
        ...
        //     Db::table('blog_product')->cache('product_list',6000)->select();
        //     $this->product_list = Cache::get('product_list');
        //     // echo '我是刚缓存';die;
        ...
        // dump($this->config_list);die;
        if(empty($this->config_list)){

            Db::table('blog_config')->where('id','neq','1')->field('name,value,remark')->cache('config_list',6000)->select();
    ...
    public function LoginRecord($type = 1) {
        switch ($type) {
            case 0:
                $type = 0;
                $remark = '【主动退出】正在退出后台系统！';
                break;
            case 1:
                $type = 1;
                $remark = '【账户登录】正在进入后台系统！';
                break;
            case 2:
                $type = 1;
                $remark = '【QQ快捷】正在进入后台系统！';
                break;
            case 3:
                $type = 0;
                $remark = '【登录过期】正在退出后台系统！';
                break;
            default:
                $type = '3';
                $remark = '【未知】';
